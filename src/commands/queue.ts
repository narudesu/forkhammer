...
import type { FeedEvent, SupabaseClientLike, SupabaseConfig } from "../worker/types";
...
export function resolveProjectFromIssueKey(config: Config, issueKey: string) {
  const projects = Object.entries(config.project ?? {}).map(([name, project]) => ({
    name,
...
function hasIssueKey(data: UltrafeedEventData): data is UltrafeedEventData & { issue_key: string } {
...
function formatEventStatus(eventType: string) {
  if (ultrafeedWorkerEmittedEventTypes.includes(eventType as (typeof ultrafeedWorkerEmittedEventTypes)[number])) {
...
function printQueueEventDetails(event: ParsedQueueEvent) {
  if (event.event_type === "issue_validated") {
    const data = event.data as UltrafeedEventData & {
      ...
      summary: string;
      todos: Array<string>;
      questions: Array<{ text: string; relatedFilePath: string }>;
      relatedFiles: Array<{ path: string; note: string }>;
    };

    console.log(chalk.green("Clarity:"), data.clarity);
    console.log(chalk.green("Summary:"), data.summary);
    console.log(chalk.green("Todos:"), data.todos.length);
    console.log(chalk.green("Questions:"), data.questions.length);
    console.log(chalk.green("Related files:"), data.relatedFiles.length);
    return;
  }
